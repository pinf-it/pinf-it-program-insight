...
exports.parse = function(programPath, options, callback) {
	...
	return PACKAGE_INSIGHT.parse(programPath, opts, function(err, programDescriptor) {
		...
		function followDependencies(descriptor, callback) {
			...
			if (options.debug) console.log("[pinf-it-program-insight] followDependencies(descriptor) - descriptor.combined.dependencies.bundled", descriptor.combined.dependencies.bundled);
			...
			for (var alias in descriptor.combined.dependencies.bundled) {
				waitfor(alias, function(alias, done) {
					...
					if (!descriptor.combined.dependencies.bundled[alias].location) {
						console.error("descriptor.raw", JSON.stringify(descriptor.raw, null, 4));
						console.error("descriptor.combined", JSON.stringify(descriptor.combined, null, 4));
						console.error("descriptor.combined.dependencies.bundled[alias]", descriptor.combined.dependencies.bundled[alias]);
						return done(new Error("'descriptor.combined.dependencies.bundled[alias].location' is not set!"));
					}

					var depPath = PATH.join(descriptor.dirpath, descriptor.combined.dependencies.bundled[alias].location);
					if (process.env.PINF_LOOKUP_CEILING) {
							if (/^\.\.\//.test(PATH.relative(PATH.dirname(process.env.PINF_LOOKUP_CEILING), depPath))) {
									if (options.debug) console.log("[pinf-it-program-insight] skip traversing up further for depPath", depPath);
									return done();
							}
					}

					return followPackage(depPath, done);
				});
			}
			return waitfor();
		}
		...
		function followPackage(uri, callback) {
			...
			if (options.debug) console.log("[pinf-it-program-insight] followPackage(uri) - uri", uri);
